...
impl BranchManager<'_> {
    ...
    fn apply_branch(
        ...
        if dbg!(merge_base != default_target.sha) {
            ...
            let new_head = if dbg!(branch.allow_rebasing) {
                let commits_to_rebase = dbg!(repo.l(branch.head, LogUntil::Commit(merge_base))?);

                let head_oid =
                    cherry_rebase_group(repo, default_target.sha, &commits_to_rebase, true)?;

                dbg!(repo.find_commit(head_oid)?)
            } else {
